...
![img](/images/thread-8/clip_image001.png)
...
![img](/images/thread-8/clip_image003.png)

![img](/images/thread-8/clip_image005.png)

所以，最终结果的计算应该是

![img](/images/thread-8/clip_image007.png)

![img](/images/thread-8/clip_image009.png)

在实际运用的时候，只有从未出现过并发冲突的时候，base基数才会使用到，一旦出现了并发冲突，之后所有的操作都只针对Cell[]数组中的单元Cell。

![img](/images/thread-8/clip_image011.png)
...
![img](/images/thread-8/clip_image013.png)

![img](/images/thread-8/clip_image015.png)
...
![img](/images/thread-8/clip_image017.png)

关键入参是函数式接口Function。它的入参是上一个阶段计算后的结果，返回值是经过转化后结果。

**消费类 thenAccept：**

![img](/images/thread-8/clip_image019.png)

关键入参是函数式接口Consumer。它的入参是上一个阶段计算后的结果， 没有返回值。

 **执行操作类 thenRun：**

![img](/images/thread-8/clip_image021.png)

对上一步的计算结果不关心，执行下一个操作，入参是一个Runnable的实例，表示上一步完成后执行的操作。

**结合转化类:**

![img](/images/thread-8/clip_image023.png)
...
![img](/images/thread-8/clip_image025.png)
...
![img](/images/thread-8/clip_image043.png)

#### 第一步

我们想从一批Circle中挑选出挑选出半径为2的圆，于是我们写了一个方法

![img](/images/thread-8/clip_image045.png)

这样，无疑很不优雅，如果我们想挑选半径为3的圆，难道还要再写一个方法？于是我们考虑将选择条件进行参数化，比如根据颜色挑选出圆或者根据半径挑选出圆

![img](/images/thread-8/clip_image047.png)

![img](/images/thread-8/clip_image049.png)
...
![img](/images/thread-8/clip_image051.png)

 在进行圆的挑选的方法里，我们把这个接口作为参数进行传递

![img](/images/thread-8/clip_image053.png)

然后，我们只要按业务需求实现接口，并传入实现类的实例即可

![img](/images/thread-8/clip_image055.png)
...
![img](/images/thread-8/clip_image057.png)

匿名内部类占用代码空间较多，而且存在着模版代码，这种情况下，Lambda表达式就可以派上用场了

![img](/images/thread-8/clip_image059.png)
...
![img](/images/thread-8/clip_image061.png)
...
![img](/images/thread-8/clip_image063.png)

再考察Callable接口和Supplier接口

![img](/images/thread-8/clip_image065.png)

![img](/images/thread-8/clip_image067.png)
...
![img](/images/thread-8/clip_image069.png)是在ArrayBlockingQueue的核心enqueue和dequeue方法中经常会用到的，这三个变量很容易放到同一个缓存行中，进而产生伪共享问题。
...
![img](/images/thread-8/clip_image071.png)
