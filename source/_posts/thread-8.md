...
![img](/images/thread-8/clip_image002.jpg)
...
![img](/images/thread-8/clip_image004.jpg)

![img](/images/thread-8/clip_image006.jpg)

所以，最终结果的计算应该是

![img](/images/thread-8/clip_image008.jpg)

![img](/images/thread-8/clip_image010.jpg)

在实际运用的时候，只有从未出现过并发冲突的时候，base基数才会使用到，一旦出现了并发冲突，之后所有的操作都只针对Cell[]数组中的单元Cell。

![img](/images/thread-8/clip_image012.jpg)
...
![img](/images/thread-8/clip_image014.jpg)

![img](/images/thread-8/clip_image016.jpg)
...
![img](/images/thread-8/clip_image018.jpg)

关键入参是函数式接口Function。它的入参是上一个阶段计算后的结果，返回值是经过转化后结果。

**消费类 thenAccept：**

![img](/images/thread-8/clip_image020.jpg)

关键入参是函数式接口Consumer。它的入参是上一个阶段计算后的结果， 没有返回值。

 **执行操作类 thenRun：**

![img](/images/thread-8/clip_image022.jpg)

对上一步的计算结果不关心，执行下一个操作，入参是一个Runnable的实例，表示上一步完成后执行的操作。

**结合转化类:**

![img](/images/thread-8/clip_image024.jpg)
...
![img](/images/thread-8/clip_image026.jpg)
...
![img](/images/thread-8/clip_image044.jpg)

#### 第一步

我们想从一批Circle中挑选出挑选出半径为2的圆，于是我们写了一个方法

![img](/images/thread-8/clip_image046.jpg)

这样，无疑很不优雅，如果我们想挑选半径为3的圆，难道还要再写一个方法？于是我们考虑将选择条件进行参数化，比如根据颜色挑选出圆或者根据半径挑选出圆

![img](/images/thread-8/clip_image048.jpg)

![img](/images/thread-8/clip_image050.jpg)
...
![img](/images/thread-8/clip_image052.jpg)

 在进行圆的挑选的方法里，我们把这个接口作为参数进行传递

![img](/images/thread-8/clip_image054.jpg)

然后，我们只要按业务需求实现接口，并传入实现类的实例即可

![img](/images/thread-8/clip_image056.jpg)
...
![img](/images/thread-8/clip_image058.jpg)

匿名内部类占用代码空间较多，而且存在着模版代码，这种情况下，Lambda表达式就可以派上用场了

![img](/images/thread-8/clip_image060.jpg)
...
![img](/images/thread-8/clip_image062.jpg)
...
![img](/images/thread-8/clip_image064.jpg)

再考察Callable接口和Supplier接口

![img](/images/thread-8/clip_image066.jpg)

![img](/images/thread-8/clip_image068.jpg)
...
![img](/images/thread-8/clip_image070.jpg)是在ArrayBlockingQueue的核心enqueue和dequeue方法中经常会用到的，这三个变量很容易放到同一个缓存行中，进而产生伪共享问题。
...
![img](/images/thread-8/clip_image072.jpg)
